...
HOTNEWS_MD = os.path.join(OUTPUT_DIR, "hotnews.md")
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _serve_markdown(self):
        if not os.path.exists(HOTNEWS_MD):
            self.send_response(200)
            self.send_header("Content-Type", "text/html; charset=utf-8")
            self.send_header("Cache-Control", "no-cache, no-store, must-revalidate")
            self.end_headers()
        ...
        with open(HOTNEWS_MD, "r", encoding="utf-8") as f:
            md_text = f.read()
    ...
    def _export_recommendations(self):
        ...
        # The recommendations section starts after the header and ends before "## 全部文章"
        lines = md_text.split("\n")
        ...
        for line in lines:
            ...
            if line.startswith("# 新闻日报"):
                recommendations_lines.append(line)
                in_recommendations = True
                continue

            # Stop when we hit "全部文章" section
            if line.startswith("## 全部文章"):
                break

            if in_recommendations:
                recommendations_lines.append(line)

        recommendations_text = "\n".join(recommendations_lines).strip()

        if not recommendations_text or "今日选题推荐" not in recommendations_text:
            self.send_error(404, "暂无推荐内容")
            return

        payload = recommendations_text.encode("utf-8")
        self.send_response(200)
        self.send_header("Content-Type", "text/markdown; charset=utf-8")
        self.send_header("Content-Disposition", "attachment; filename=recommendations.md")
        self.send_header("Content-Length", str(len(payload)))
        self.end_headers()
        self.wfile.write(payload)
